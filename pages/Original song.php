    ...
    <article id="content">
      ...
      <div class="col-2">
        ...
        <h2>Khmer original</h2> 
         ...
         <?php
       require_once('dbconfig.php');
       $sql = "select * from production where type = 'original' and nationality = 'khmer' ORDER BY vol desc limit 0, 15";
       $result = $conn->query($sql);
       while ($row = $result->fetch_object()) {
         echo "<div class='gallery'>
           <a class='' href='audio1.php? vol=$row->vol&amp;album=$row->name&amp;id=$row->id&amp;view=$row->view'>
              <img class='sizef' alt='Khmer Original  $row->name' src='../images/$row->local_img' >  
