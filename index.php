...
<div id="main">
  <header>
    <nav>
      <ul>
        ...
        <li><a href="pages/Khmer.php">Khmer Song</a></li>
        <li><a href="pages/video.php">Video</a></li>
        <li><a href="pages/English.php">English Song</a></li>
        <li><a href="pages/Thai.php">Thai Song</a></li>
  ...
  <article id="content">
   <div class="col-1" >
          <form action="pages/search_result.php" id="subscribe" method="POST">
        <fieldset>
          <label>
      ...
      <h2>Most Popular</h2>          
            ...
            <?php  
                require_once('pages/dbconfig.php');
                $sql = "select * from production ORDER BY view desc limit 0, 5";
                $result = $conn->query($sql);
                   while ($row = $result->fetch_object()) {
                    echo "<div class='p2'>
            <img src='images/$row->local_img' class='sizefe' alt=''> <a href='pages/audio1.php? vol=$row->vol&amp;album=$row->name&amp;id=$row->id&amp;view=$row->view' class='more'>Read More</a></div>";
                   }
            ?>
    ...
    <h2> Latest Album </h2>
       ...
       $sql = "select * from production ORDER BY id desc limit 0, 9";

       $result = $conn->query($sql);
       while ($row = $result->fetch_object()) {
        if ($row->type == 'khmer original'){
          //pass value view and id
          echo "<div class='gallery'>
           <a class='' href='pages/audio1.php? vol=$row->vol&amp;album=$row->name&amp;id=$row->id&amp;view=$row->view' >
              <img class='sizef' alt='$row->name' src='images/$row->local_img' >  
                <div class='desc' >$row->name</div> 
          </a>
         </div>"; 
        }
         else if($row->name == 'english original') {
            echo "<div class='gallery'>
           <a class='' href='pages/audio1.php? vol=$row->vol&amp;album=$row->name&amp;id=$row->id&amp;view=$row->view' >
              <img class='sizef' alt='$row->name' src='images/$row->local_img' >  
                <div class='desc'> $row->name </div> 
         ...
         else if($row->name == 'thai original') {
            echo "<div class='gallery'>
           <a class='' href='pages/audio1.php? vol=$row->vol&amp;album=$row->name&amp;id=$row->id&amp;view=$row->view' >
              <img class='sizef' alt='$row->name' src='images/$row->local_img' >  
                <div class='desc'> $row->name </div> 
